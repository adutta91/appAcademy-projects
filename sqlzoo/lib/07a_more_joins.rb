...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      styles.style, album_tracks.price/album_tracks.num_tracks
    FROM
      styles
    JOIN
      (
        SELECT albums.*, COUNT(tracks.*) AS num_tracks
        FROM albums
        JOIN tracks ON tracks.album = albums.asin
        WHERE albums.price IS NOT NULL
        GROUP BY albums.asin
      ) album_tracks ON styles.album = album_tracks.asin
    GROUP BY
      styles.style, album_tracks.price
    ORDER BY
      album_tracks.price/album_tracks.num_tracks
    LIMIT 5
