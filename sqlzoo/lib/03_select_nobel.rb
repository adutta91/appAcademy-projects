...
def literature_1962
  ...
  execute(<<-SQL)
    ...
    FROM
      nobels
    WHERE
      nobels.yr = 1962 AND nobels.subject = 'Literature'
  SQL
end
...
def einstein_prize
  ...
  execute(<<-SQL)
    SELECT
      nobels.yr, nobels.subject
    FROM
      nobels
    WHERE
      nobels.winner = 'Albert Einstein'
  SQL
end
...
def millennial_peace_prizes
  ...
  execute(<<-SQL)
    ...
    FROM
      nobels
    WHERE
      nobels.yr >= 2000 AND nobels.subject = 'Peace'
  SQL
end
...
def eighties_literature
  ...
  execute(<<-SQL)
    ...
    FROM
      nobels
    WHERE
      nobels.yr BETWEEN 1980 AND 1989 AND nobels.subject = 'Literature'
  SQL
end
...
def presidential_prizes
  ...
  execute(<<-SQL)
    ...
    FROM
      nobels
    WHERE
      nobels.winner IN ('Theodore Roosevelt', 'Woodrow Wilson', 'Jimmy Carter')
  SQL
end
